...
// Mengambil product_id dari URL. Jika tidak ada, default ke 7 (seperti Pasteria Phone Strap)
$product_id = isset($_GET['id']) ? intval($_GET['id']) : 7;
$product = null;
$variants = [];
...
<head>
  ...
  <title>Jepitakulucu - <?php echo htmlspecialchars($product['name']); ?></title>
...
<header>
  ...
  <div class="container-section">
    <div class="container">
      <div class="image-section">
        ...
        <p class="caption"><?php echo htmlspecialchars($product['name']); ?></p>
      </div>
        ...
        <div class="colors">
          <?php if (!empty($variants)): ?>
            <?php foreach ($variants as $variant): ?>
              <div class="color-box" title="<?php echo htmlspecialchars($variant); ?>" style="background-color: <?php echo htmlspecialchars($variant); ?>;"></div>
              <?php endforeach; ?>
          <?php else: ?>
            <div class="color-box" style="background-image: url('assets/assets/image/color_placeholder.jpg');"></div>
          <?php endif; ?>
        </div>
  ...
  <script>
    ...
    fetch("add_to_cart.php", { // Pastikan path ke add_to_cart.php ini benar
      method: "POST",
    ...
    .then(data => {
      if (data.success) {
        ...
        fetch("get_cart_count.php") // Pastikan path ke get_cart_count.php ini benar
        .then(res => res.text())
        .then(count => {
          // Asumsi ada elemen dengan class 'cart-count' di header Anda
        ...
        if (data.message === "Anda belum login.") {
          window.location.href = "login.php"; // Pastikan path ke login.php ini benar
        }
      }
    });
  }
